...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < 80; ++i) {
        ...
        if (hit_max < hit_histogram[i]) {
            hit_max = hit_histogram[i]; // find out the frequency of hit time
                                        // with max occurences
            hit_max_i = i;              // corresponding hit time
        }
        if (miss_histogram[i] > 3 && miss_min_i == 0)
            miss_min_i = i; // find the first min time which has occured for
                            // more than 3 times
    }
    printf("hit_max_i is: %ld\n", hit_max_i);
    printf("miss_min_i is: %ld\n", miss_min_i);
    // the threshold should be set such that it miss_min_i < threshold <
    // hit_max_i
    ...
    for (int i = hit_max_i; i < miss_min_i; ++i) {
        if (min > (hit_histogram[i] + miss_histogram[i])) {
            min = hit_histogram[i] +
                  miss_histogram[i]; // to find the index where the hit and miss
                                     // cycles = 0 for avoiding false positives
            min_i = i;
        }
    }
    printf(
