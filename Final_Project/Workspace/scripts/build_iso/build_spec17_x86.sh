...
cp $BASE/scripts/build_iso/configs_cpu17/gcc-linux-x86_all.cfg SPEC17_workloads/config

cd SPEC17_workloads

source shrc

runcpu --config=gcc-linux-x86_all.cfg --action=build all
runcpu --config=gcc-linux-x86_all.cfg --action=run --fake all
