...
public:
    ...
    uint32_t lru_victim_WAY_PARTITION(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type);
    void lru_update_WAY_PARTITION(uint32_t set, uint32_t way, uint32_t cpu);
};
